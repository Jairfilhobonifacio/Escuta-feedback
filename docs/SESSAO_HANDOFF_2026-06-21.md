...
| **C** Board reordenar intra-coluna (P1) | ✅ **backend + front feitos e no `master`** (`fad3e6c`, deployado 22/06); **falta SÓ o QA visual do gesto** (`docs/SPEC_BOARD_REORDER_FRONT.md`) |
...
5. **Deploy** dos novos commits p/ prod (Modal API + Vercel painel) — ✅ **FEITO em 21–22/06** (re-deploy da `fad3e6c`; login de operador validado E2E). Ver seção "cont." abaixo.
...
**Merge:** `worktree-board-front-dnd` → `master` em **`fad3e6c`** (pushed; `master` = `origin/master`).
**DEPLOYADO em 22/06** (re-deploy da `fad3e6c`): **API Modal `escuta-api`** + **painel Vercel
`escuta-feedback.vercel.app`**. Login de operador validado E2E (login 200+token · `/api/auth/me`=200 ·
fail-closed 401 sem auth · senha errada 401). **Falta SÓ o QA visual do gesto DnD** (pode ser feito
direto em prod — ver "Pendências"). Trabalho guiado por **2 rounds de review adversarial**
(Ultracode: 21 + 8 agentes) — foi o que salvou a feature, que tinha premissas falhas na spec original.
